...
class FileLoader {
   ...
   * @protected
   */
  _parseDefinitionsSync (services = []) {
    for (const id in services) {
      if (id === '_defaults') {
        this._parseDefaultsSync(services._defaults)
      } else {
        this._parseDefinitionSync(services, id)
      }
    }
  }
  ...
  async _parseDefinition (services, id) {
    ...
    if (typeof service === 'string') {
      ...
      this.container.setAlias(id, service.alias)
   ...
   * @private
   */
  _parseDefinitionSync (services, id) {
    const service = services[id]
  ...
  _parseDefaultsSync (defaults = {}) {
    if (!defaults || !defaults.autowire) {
      return
    }
    throw new Error('Autowire is not supported in synchronous mode. Use the async load() method instead.')
  }
   ...
   * @protected
   */
  _parseImportsSync (imports = []) {
    for (const file of imports) {
      const workingPath = this.filePath
      this.loadSync(path.join(path.dirname(this.filePath), file.resource))
      this.filePath = workingPath
    }
  }
   ...
   * @protected
   */
  _parseParametersSync (parameters = {}) {
    for (const key in parameters) {
      this._container.setParameter(key, parameters[key])
    }
  }
